...
(async () => {
  ...
  await p.goto('http://localhost:8094/x.html');
  ...
  /* 事务所一屏最多列三个案子，c07 还排不上。直接接案。 */
  if(await p.$('.caseCard[data-id="c07"]')) await p.click('.caseCard[data-id="c07"]');
  else await p.evaluate(()=>takeCase('c07'));
  ...
  // 证人
  console.log('\n—— 证人 ——');
  const WQ = ['你买药的钱是怎么付的，打给谁？',
              '他收过你的好处没有，加过价没有？',
              '要是买不起正版、又断了这个渠道，会怎么样？'];
  for (const q of WQ){
  ...
  console.log('\n—— 辩论 ——');
  const LINES = [
   '被告人处于购买一方。银行流水表明货款从未经过他的账户，订单价与病友汇款额逐笔一致，没有加价。他提供的是渠道、汇款方法和翻译，不是卖方。',
   '刑法上的销售以营利为目的为核心。被告人不收取任何费用，他自己也在这份订购名单上。不以营利为目的的帮助购买，不能等同于销售。',
   '涉案药品主成分为甲磺酸伊马替尼，含量符合标示量，色谱图与正版对照品一致。所谓假药是未经批准进口的法律拟制，不是事实上的假药。',
   '正版年费用二十八万，涉案药品两千。一旦定罪，一千余名病友立刻断药。定罪的后果与刑法所要保护的法益相背离，本案没有刑罚必要性。'];
  ...
  await p.screenshot({path:'shot-c07.png'});
